...
const PORTFOLIO_DATA = {
  ...
  skills: {
    ...
    soft: ['Ownership', 'Team Collaboration', 'Punctuality'],
  },
  experience: [
    {
  ...
  projects: [
    {
      name: 'TR42',
      description:
        'An editing product to facilitate seamless translation of documents across multiple languages, enhancing efficiency and accuracy for translation projects',
      features: [
...
export function AIChat() {
  ...
  const generateResponse = (query: string): string => {
    ...
    if (
      ...
      return `Pankaj is skilled in ${PORTFOLIO_DATA.skills.technical.join(
        ', '
      )}. He also works with tools like ${PORTFOLIO_DATA.skills.tools.join(
        ', '
      )}.`;
    } else if (
      ...
      }. Key features include: ${PORTFOLIO_DATA.projects[0].features.join(
        ', '
      )}. It was built using ${PORTFOLIO_DATA.projects[0].tech}.`;
    } else if (query.includes('ezworks') || query.includes('ez works')) {
      return `EZ Works is ${
        PORTFOLIO_DATA.projects[1].description
      }. Key contributions include: ${PORTFOLIO_DATA.projects[1].features.join(
        ', '
      )}. It was built using ${PORTFOLIO_DATA.projects[1].tech}.`;
    } else if (query.includes('redbasil')) {
      return `Redbasil is ${
        PORTFOLIO_DATA.projects[2].description
      }. Key contributions include: ${PORTFOLIO_DATA.projects[2].features.join(
        ', '
      )}. It was built using ${PORTFOLIO_DATA.projects[2].tech}.`;
    } else if (
